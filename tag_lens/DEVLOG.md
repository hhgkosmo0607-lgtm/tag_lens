...
오늘(2026-07-09) 완료: 카메라 시계 오류 감지 + 경계 안전장치(daynight), 얼굴 검출
임계값 조정, 그리고 제일 큰 변화로 genre/indoor/daynight-폴백/animal 판별을
전부(4개 다) 자체 학습 CNN·ImageNet에서 CLIP 제로샷으로 교체함(실내/실외 오탐
14→1건, 동물 판별 최종 11/11, 자연 장르 daynight 5건 중 4건 해결, 장르는
82장 중 35장(42.7%) 변경 — 대부분 개선이나 확신도 임계값 때문에 `#기타`로
정보 손실된 케이스도 15건 있음). 재학습 시도 1번은 검증셋 점수만 믿고 바로
반영했다가 실사진 기준 13.3%로 역행하는 사고가 나서 롤백하고 CLIP으로 방향을
바꿨음. 동물 오탐은 절대 임계값만으론 못 거른 3건을 "종 프롬프트 1등-2등 점수차
(gap)" 신호로 추가 방어(gap<2.5면 제외) — 82장 중 정확히 1건만 고쳐짐. 실사용
테스트(서버 띄우고 82장 업로드)로 전부 실측 발견/검증함. README.md도 CLIP 전환
상태에 맞춰 갱신 완료.
남은 것들:
1. daynight `04541da4`(밤바다) 회귀 — CLIP이 이 사진 하나에서만 CNN보다 약함,
   CNN+CLIP 앙상블 검토 여지
2. daynight CNN(폴백용)이 원래 갖고 있던 EXIF "+00:00 아닌" 카메라 시계 오류
   유형(`5b9c3f20`, 경계 안전장치도 못 잡음) — 패턴이 더 모이면 재설계 검토
3. 장르 CLIP 전환으로 생긴 `#기타` 정보 손실 15건 — 클래스당 프롬프트를
   늘리거나 임계값 조정 검토 여지 있음, 사용자가 브라우저로 직접 확인 후 재검토
   하기로 보류 중
4. `도시`/`인물`의 벽/간판 클로즈업 등 실내/실외 오탐 — HAKATA 간판처럼 CLIP도
   못 맞추는 잔여 케이스 소수 남음
5. "학습 버튼"(사용자가 태그 직접 수정 → 재학습) 기능 설계만 하고 미착수 —
   검증 게이트(실사진 테스트 통과해야 배포) 필수로 넣어야 함
6. Late Fusion(메타 분류기) 아이디어 사용자가 제안, 기록만 하고 보류 — 지금
   규모(오탐 케이스별 1~3건)엔 학습 데이터가 부족해서 시기상조
7. 실내/실외 데이터셋 후보 기록해둠(prithivMLmods/IndoorOutdoorNet,
   Google Street View 등) — 착수는 안 함
8. 텀블러/음료 사물 vs 음식 애매 케이스 — 방향만 정해짐, 착수 안 함
9. git 커밋 아직 안 함(오늘 변경량 많음)
10. 환경 메모: 실제 런타임은 conda `firstvenv`, 시스템 기본 python(3.14)이
    아님 — 헷갈리지 말 것 (자세한 사고 경위는 위 "다음 할 일" 마지막 항목)
