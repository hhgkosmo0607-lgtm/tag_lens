...
from ai.daynight_predict import extract_exif_hour, get_exif_datetime_and_offsets, resolve_daynight
...
def extract_photo_date(image_path: str) -> str:
    ...
    """
    date_str, _, _ = get_exif_datetime_and_offsets(image_path)
    if date_str:
        try:
...
def upload():
    ...
    for file in files:
        ...
        try:
            ...
            exif_hour, raw_exif_hour = extract_exif_hour(str(save_path))
            daynight, daynight_probs = resolve_daynight(
...
@app.route("/photo/<int:photo_id>/tag/add", methods=["POST"])
def add_tag(photo_id: int):
    photo = get_photo(photo_id)
    if photo is None:
        return render_template("404.html"), 404
