...
> "중요한 설계 원칙 하나를 짚고 가겠습니다 — 이 프로젝트는 모델을 처음부터 학습시키는 대신, **대규모로 사전학습된 모델을 프롬프트 엔지니어링과 다층 검증 로직으로 정교하게 통제하는 아키텍처**를 택했습니다. 재학습 파이프라인에 들였을 시간과 자원을, 판단 기준을 설계하고 오탐을 막는 로직에 그대로 투입한 겁니다. 그래서 판단 기준을 바꾸고 싶으면 GPU도 학습 데이터도 필요 없이 프롬프트 문장 하나만 고치면 됩니다."
